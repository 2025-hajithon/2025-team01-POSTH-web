...
const Header: React.FC<{ onBack?: () => void }> = ({ onBack }) => (
  <div className="flex items-center px-6 pt-10 pb-4">
    <button
      onClick={onBack}
      className="p-2 hover:bg-white/10  bg-transparent rounded-full transition-colors focus:outline-none"
    >
  ...
  return (
    <div className="h-full w-full bg-[#3a3b49] text-white flex flex-col">
      ...
      <div className="flex-1 px-7 flex flex-col">
        {/* 사용자 인사말 */}
        <div className="mb-10">
          <h1 className="text-[26px] leading-[1.5] text-white-100">
            <span className="font-extrabold">{nickname || "..."}</span>
            <span className="font-medium"> 님의</span>
          </h1>
          <h2 className="text-[24px] text-white-100 font-semibold leading-[1.3] mt-1">
            마이페이지
          </h2>
        </div>
